...
`📌X,X,X ONLY FEATURE` 표기가 되지 않는 기능들은 모든 브라우저에서 지원하는 기능입니다. 

---

## 읽은 후 생각
> 개발 시에 크롬을 사용해야한다는 말을 어디서 들었는데, 왜 그런지에 대한 이유를 하나 알려주는 듯한 느낌이었다. (디버깅 툴 풍부..)
> 프로젝트들에 활용해볼 만한 몰랐던 기능들이 많이 있는 거 같다. 특히 페이지가 변경되거나, 커서 위치에 민감한 요소들 등을 디버깅하기에는 어려움이 꽤 있었는데, 오늘 아티클을 활용해서 디버깅을 하면 유용할 것 같다. 
